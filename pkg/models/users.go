...
func prepareUserSelectStmt(tx Preparer, option string) (*sql.Stmt, error) {
	sel := generateSelectString(
		"users",
		[]string{"id", "first_name", "last_name", "username", "join_date"},
	)
	if checkOptionSqlString(option, "WHERE") {
		return tx.Prepare(sel + " " + option)
	}
	return tx.Prepare(sel)
}

func prepareUserInsertStmt(tx Preparer, returning string) (*sql.Stmt, error) {
	ins := generateInsertString(
		"users",
		[]string{"id", "first_name", "last_name", "username", "hash", "join_date"},
		[]string{"uuid_generate_v4()", "$1", "$2", "$3", "$4", "now()"},
	)
	if checkOptionSqlString(returning, "RETURNING") {
...
func (u *User) fetchRow(option string, args ...interface{}) error {
	stmt, err := prepareUserSelectStmt(db, option)
	if err != nil {
		return err
	}
	defer stmt.Close()
